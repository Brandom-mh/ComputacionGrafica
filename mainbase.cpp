...
int main()
{
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		// Tiempo actual desde que inició GLFW (en segundos)
	
		time_t ahora = time(NULL);
		if (ahora != ultimoCambio) {
			ultimoCambio = ahora;
			idxColor = (idxColor + 1) % 3;

			if (idxColor == 0) { rojo = 1.0f; verde = 0.0f; azul = 0.0f; }   
			else if (idxColor == 1) { rojo = 0.0f; verde = 1.0f; azul = 0.0f; } 
			else { rojo = 0.0f; verde = 0.0f; azul = 1.0f; }                   		}
